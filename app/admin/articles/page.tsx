...
const STATUS_OPTIONS: { value: CMSStatus | "all"; label: string }[] = [
  { value: "all", label: "All" },
...
export default function ArticlesPage() {
  const { articles, hydrated, update, remove } = useCMS();
        ...
        <div className="flex gap-1 rounded-lg border border-white/8 bg-[#111827] p-1">
          {STATUS_OPTIONS.map((o) => (
            <button
              key={o.value}
              type="button"
              onClick={() => setStatusFilter(o.value as CMSStatus | "all")}
              ...
              {filtered.map((a) => (
                <tr key={a.id} className="group transition hover:bg-white/3">
                  ...
                  <td className="px-4 py-3.5">
                    <select
                      value={a.status}
                      onChange={(e) => update(a.id, { status: e.target.value as CMSStatus })}
                    ...
                    <div className="flex items-center justify-end gap-2">
                      ...
                      {confirmDelete === a.id ? (
                        <div className="flex gap-1">
                          <button
                            type="button"
                            onClick={() => { remove(a.id); setConfirmDelete(null); }}
